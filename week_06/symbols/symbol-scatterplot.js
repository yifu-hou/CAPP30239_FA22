...
(function symbol() {
  ...
  d3.csv('penguins.csv').then(data => {
    ...
    svg.append("g")
        ...
        .data(data)
        .join("path")
        .attr("d", d3.symbol().type(d3.symbolCross).size(12))  // design the style of the symbol
        .attr("class", "cross")
        .attr("fill", "orange")
        .attr("opacity", 0.75)
        .attr("transform", d => `translate(${x(d.body_mass_g)},${y(d.flipper_length_mm)})`);  // give the x and y position for each symbol
